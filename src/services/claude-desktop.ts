...
export class ClaudeDesktopOrchestrator {
  ...
  private async executeStep(
    ...
    try {
      ...
      if (step.action === 'navigate' || step.action === 'click' || step.action === 'verify') {
        const screenshot = await this.takeScreenshot(scenarioDir, `step_${stepIndex}_${step.action}`);
        if (screenshot) {
  ...
  private async startVideoRecording(scenarioDir: string, scenarioId: string): Promise<string | null> {
    try {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
